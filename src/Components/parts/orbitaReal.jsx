...
const scale = 100; // 1 UA = 100 unidades en el espacio 3D
...
const generateOrbitPoints = (a, e, steps = 360) => {
  ...
  for (let i = 0; i < steps; i++) {
    ...
    // Coordenadas en el plano XZ, aplicando el factor de escalado
    const x = r * scale * Math.cos(ν);
    const z = r * scale * Math.sin(ν);
...
planetsData.forEach((planet) => {
  ...
  label.position.set(0, planet.radius + 5, 0); // Posicionar por encima del planeta
